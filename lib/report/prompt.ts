...
## 未来予想
冒頭で、**市場のコンセンサス（現在の株価・PER等の現在値に織り込まれていると考えられる市場の期待）と、上記の実データ分析から導かれる見立てとの差分**を1〜2文で述べること（例:「市場は◯◯を懸念しすぎ／楽観しすぎている可能性がある」）。これは断定ではなく「可能性がある」という程度に留め、根拠となる引用元番号[n]を必ず付す。**良い会社であることと良い投資であることは別である**（優れた事業でも株価に既に織り込まれていれば投資妙味は別問題）という視点を踏まえること。
その上で、**足元の状況（直近ニュース・現在のマクロ/市場環境＝金利・VIX・指数・ドル指数・原油の現在水準）を加味した上で**、実データとAI推論に基づく今後のシナリオ（強気・中立・弱気の3シナリオ）を提示する。各シナリオには次を必ず添える: (a) 条件・目安となる価格/指標水準、(b) そのシナリオが実現するために鍵となる**カタリスト**（次の決算発表、新製品/新事業、マクロ環境の転換、金利・為替の変化など、実データまたは既知の一般情報の範囲に限る）。各シナリオの根拠に引用元番号[n]を付す。確定していない噂・未発表の具体的な日付や製品名は挙げず、一般論として述べる場合はその旨（例:「一般に想定される」）を明示すること。
主要なリスクを挙げる際は、必ずそのリスクの**緩和材料・条件（リスクが顕在化しにくくなる/影響を和らげる根拠）とペア**で書くこと。リスクの列挙だけで終わらせない。
セクションの最後に、**「弱気シナリオが実現しても、現在の株価水準は正当化されうるか」という一言判定**（安全域の有無・程度を実データに基づき言語化したもの）を明記する。
**これは実データに基づくAIの推論であり予言ではないこと、投資判断はユーザー自身の責任であることを冒頭に明記する。架空の確率や具体的な騰落率（例:「70%の確率で+20%」）は絶対に作らない**。シナリオはあくまで実データ・引用元[n]に基づく条件付きの定性的な筋立てとし、「過去の傾向に基づくと」等の枠を明示すること。
...
- **投資テーゼとしての未来予想**: 「未来予想」は単なる良い会社の紹介で終わらせず、市場コンセンサスとの差分（可能性としての表現＋引用元[n]）・各シナリオのカタリスト・リスクと緩和材料のペア・弱気耐性の一言判定（安全域の言語化）を必ず含める。
- **架空の確率・具体的な騰落率の禁止**: 「70%の確率で+20%」のような具体的な確率や騰落率を作り出してはならない（それは実際のバックテストの実測勝率データに基づく別機能の担当であり、本レポートの推論では扱わない）。シナリオは実データ・引用元[n]に基づく条件付きの定性的な筋立てに留める。
