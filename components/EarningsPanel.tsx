...
export function EarningsPanel({ symbol }: Props) {
  ...
  return (
    <div className="bg-panel border border-border rounded-xl p-5">
      <h2 className="text-ink font-semibold mb-4">決算情報</h2>
      ...
      {data?.epsHistory && data.epsHistory.length > 0 && (
        <div>
          <p className="text-muted text-sm mb-2">EPS 実績 vs 予想（直近4四半期）</p>
                ...
                {data.epsHistory.map(q => {
                  ...
                  return (
                    <tr key={q.date} className="hover:bg-surface/50">
                      <td className="py-2 text-ink-2 font-mono text-sm tabular-nums">{q.date}</td>
                      <td className="py-2 text-right text-ink font-mono">
