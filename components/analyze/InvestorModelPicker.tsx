...
export default function InvestorModelPicker({ onConditionChange }: InvestorModelPickerProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* モデル選択 */}
      <div>
        <label className="block text-sm text-muted mb-2">投資家モデル（1つ選択）</label>
      ...
      <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
        <p className="text-sm text-blue-700 font-medium mb-1">投資理論・信念（この条件の土台）</p>
        <p className="text-sm text-ink leading-relaxed">{selected.philosophy}</p>
      </div>

      {/* 条件の根拠（信念からなぜこの指標が導かれるか） */}
      <div>
        <p className="text-sm text-muted mb-1.5">この信念から導かれた条件の根拠</p>
        <ul className="text-sm text-ink-2 space-y-1 list-disc list-inside leading-relaxed">
      ...
      {/* 実際の条件（結果表示と同じ形式） */}
      <div>
        <p className="text-sm text-muted mb-1.5">実際に検証する条件</p>
        <ul className="text-sm text-ink-2 space-y-1 list-disc list-inside leading-relaxed">
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
        <p className="text-sm text-amber-700 font-medium mb-1">近似についての注記（信念と実装のギャップ）</p>
        <ul className="text-sm text-amber-700/90 space-y-1 list-disc list-inside leading-relaxed">
