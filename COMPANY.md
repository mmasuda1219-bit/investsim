...
12. **4段階の流れが主役、心臓は `/trade`**: 見る（`/watch`）→まねる（`/learn`）→やる（`/trade`）→振り返る（`/review`）の流れそのものを主役とし、その中で利用者自身が理由を書いて売買する `/trade` を最も重く扱う。`/watch`（AIのリアルタイム判断を読む。旧 `/ai-session`）は入口であり比較対象。過去データのバックテスト（`/simulate`）は補助機能。前向き（リアルタイム）視点が主である点は変わらない。（2026-09-11 オーナー確認済み。旧文言「`/ai-session` を中心機能とする」から変更。見た目の規則は `DESIGN.md`）

### オーナーへの伝え方（2026-09-08 オーナー指示・全役職が従う）

13. **分かるように説明する**: オーナーは開発の専門家ではない。専門用語を使うのは構わないが、**使ったら必ずその直後に平易な説明を添える**（例:「fast-forward（早送り＝枝分かれが無いので、そのまま追いつくだけのマージ）」）。用語を出しっぱなしにしない。
14. **現在地を必ず示す**: オーナーが「自分がいまどこまで進んでいるのか」を常に把握できるようにする。応答のたびに **(a) 全体のどのステップにいるか (b) 完了したこと (c) 次にオーナー自身がやること** が分かる形にする。長い作業ほど省略しない。
15. **オーナーの作業と私の作業を分ける**: 手順を示すときは「私がやること」と「オーナーが手で行うこと」を必ずラベルで区別する。オーナーを待たせている場合は、何を待っているかを明示する。

> この3つは投資の話ではなく**開発の進め方**についての原則であり、原則1〜12と同じ強さで常に適用する。忘れやすいので、長い作業の途中でも各応答で意識し直すこと。
...
### 記録ファイルの役割分担（重複させないこと）

| ファイル | 読む相手 | 何を書くか | 書く人 |
|---|---|---|---|
| `PROGRESS.md` | 明日の自分 | その日の作業ログと再開ポイント | secretary |
| `DECISIONS.md` | 判断を検証する人 | 1つ1つの決定の理由・却下案・不変条件（ADR-lite） | architect |
| `HISTORY.md` | **他人と未来の自分** | **「いつ・何を・なぜ作ったか」の通して読める物語**。オーナーが完成時に全過程を説明できるようにするためのもの（2026-09-08 オーナー指示） | MC |
| `NEXT-STEPS.md` | 次のセッション | 順序つきの残作業と再開ポイント | MC |

**`HISTORY.md` の更新規則**: 日次では書かない（それは `PROGRESS.md` の仕事）。**1スライス出荷ごとに該当章へ1段落足す**。読者は「オーナー自身／投資家・取引先／将来の開発者」の3者なので、専門用語には原則13どおり必ず説明を付ける。
